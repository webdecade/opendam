<div id="admin-tag-file-list-page" class="span12">
	<?php
		draw_breadcrumb(array(
			...
			array("link" => path("@custom_tag_file_list", array("tag" => $tag->getId())), "text" => $tag->getName()),
			//array("link" => path("@custom_tag_file_list", array("tag" => $tag->getId())), "text" => __("Files")),
		));
	?>
	...
	<?php echo pagination($files, "@custom_tag_file_list");?>
</div>