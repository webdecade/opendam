<?php
	...
	if($has_access) {
		...
		if($request = RequestPeer::getRequestFolder($folder->getId(), $sf_user->getId())) {
			$url = "href='".url_for('@request_cancel_folder?folder_id='.$folder->getId())."' rel='facebox'";
		}
		else {
...
<div class="group-folder item-favorite folder item-folder item <?php echo $has_access ? "open" : ""; ?> <?php echo $moveDnd ? "moveDnd" : ""; ?>" data-id="folder_<?php echo $folder->getId(); ?>" data-value="<?php echo $folder->getId(); ?>">
	<div class="contain">
		<a href="<?php echo $url; ?>">
			<div class="thumbnail">
