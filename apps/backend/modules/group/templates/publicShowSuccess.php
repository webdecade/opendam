<?php
	...
	$files = $files ? $files->getRawValue() : null;
	$breadcrumb = $breadcrumb->getRawValue();
?>

<div class="container">
	<div class="row">
		<div class="span7 title-folder">
			<ul class="breadcrumb">
				<?php
					$count = 1;
					if(count($breadcrumb) > 2)
					{
								...
								foreach($breadcrumb as $folderB)
								{
									if($count > 2)
									{
										if($count == count($breadcrumb))
											echo "<li><a href='".url_for("group/publicShow?link=".$permalink->getLink())."'><i class='icon-folder-close'></i> ".$folderB."</a></li>";
										else
											echo "<li><a href='".url_for("group/publicShow?link=".$permalink->getLink()."&folder_id=".$folderB->getId())."'><i class='icon-folder-close'></i> ".$folderB."</a></li>";
									}
						...
						// Last
						echo "<li><a href='".url_for("group/publicShow?link=".$permalink->getLink()."&folder_id=".$breadcrumb[count($breadcrumb) - 2]->getId())."'>".$breadcrumb[count($breadcrumb) - 2]."</a><span class='divider'>/</span></li>";
						...
						foreach($breadcrumb as $folderB)
						{
							if($count == count($breadcrumb))
							{
								if($files)
									...
									echo "<li class='active'>".$folderB."<span class='content'>".$group->getNumberOfFiles()." <i class='icon-picture'></i> ".count($folders)." <i class='icon-folder-close'></i></span></li>";
							}
							elseif($count == 1)
								echo "<li><a href='".url_for("group/publicShow?link=".$permalink->getLink())."'>".$folderB."</a><span class='divider'>/</span></li>";
							else
								echo "<li><a href='".url_for("group/publicShow?link=".$permalink->getLink()."&folder_id=".$folderB->getId())."'>".$folderB."</a><span class='divider'>/</span></li>";
		...
		<div class="span5 title-actions">
			<ul class="inline">
				...
				<?php endif; ?>
				<?php if($files && $files->getNbResults()) : ?>
			...
			<div id="folders">
				<?php foreach($folders as $folderIn) : ?>
					<div class="folder item-folder item">
						<a class="contain" href="<?php echo url_for("group/publicShow?link=".$permalink->getLink()."&folder_id=".$folderIn->getId()); ?>">
							<div class="thumbnail">
								<?php
...
<?php if($files && $files->getLastPage() > 1) : ?>
	<div id="nav">
		<a href="<?php echo url_for("group/publicShow?link=".$permalink->getLink()."&folder_id=".$folder->getId()."&page=".$files->getNextPage()); ?>"><?php echo $files->getNextPage(); ?></a>
	</div>
<?php endif; ?>
			...
			jQuery("#download_folder").bind("click", function() {
				if(!jQuery("#download_folder").hasClass("active"))
				{
					jQuery("#download_folder").find("span").fadeOut(400, function() {
						...
						document.location.href = "<?php echo url_for("download/downloadFolder?folder_id=".$folder->getId()."&permalink_id=".$permalink->getLink()."&definition=".($permalink->getFormatHd() ? "original" : "web")); ?>";
					});
				}
			});
			/*************/
