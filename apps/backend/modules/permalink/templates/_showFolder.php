	...
	<?php if($showLabel) : ?>
		<div class="right">
			<select name="state_permalink_<?php echo $folder->getId(); ?>" id="state_permalink_<?php echo $folder->getId(); ?>" style="width: 150px;">
				...
				<option value="desactivate" <?php echo !$permalink ? "selected" : ""; ?>><?php echo __("Deactivate"); ?></option>
		...
		<?php if(!$showLabel) : ?>
			<?php if($role) : ?>
				<div style="margin: auto; width: 180px; <?php echo $permalink->getState() != PermalinkPeer::__STATE_DISABLED ? "" : "display: none;" ?>" id="div_disabled_<?php echo $folder->getId(); ?>">
					...
					<a href="javascript: void(0);" class="but_admin" id="delete_permalink_<?php echo $folder->getId(); ?>"><span><?php echo __("Deactivate"); ?></span></a>
