 ...
 */
class searchdateActions extends sfActions
{  
  ...
  public function executeGetFotoliaResults()
  {
	...
	public function executeSearch()
	{
		...
		$perPage = 25;
		//echo $this->getRequestParameter("top_keyword");exit;
		$tags = TagPeer::fetchByKeyword($this->getRequestParameter("top_keyword"));
		$selected_tag_ids = $this->getRequestParameter("selected_tag_ids", Array());
		if(trim($this->getRequestParameter("top_keyword")) != ""){
			foreach($tags as $tag){
				if(strtolower(trim($tag->getTitle())) == strtolower(trim($this->getRequestParameter("top_keyword")))){
		...
		$replacements = Array(" ", " ", " ");
		//if(strpos($this->getRequestParameter("top_keyword"),'"') !== false){
		//	$keywords = explode('"', $this->getRequestParameter("top_keyword"));
		//	$keywords = array_filter($keywords);
		//}else{
			$keywords = explode(" ", preg_replace($patterns, $replacements, $this->getRequestParameter("top_keyword")));
		//}
		//print_r($this->getRequestParameter("selected_tag_ids", Array()));exit;
