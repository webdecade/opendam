					...
					<?php foreach($files as $file) : ?>
						...
						<?php if($absolute && $relative) : ?>
							<?php
								...
								if (empty($download)) {
									$download = path("public_selection_file_download", array("code" => $basket->getCode(), "file" => $file->getId()));
								}
	...
	jQuery(document).ready(function() {
		...
		jQuery('#slider').flexslider({
			...
			after: function(slider) {
				...
				var url = Routing.generate("public_selection_file_download", {"code": "<?php echo $basket->getCode()?>", 
					"file": jQuery(slider).find(".flex-active-slide").attr("id")});
				
				jQuery("a.download").attr("href", url);
			} 
		});
