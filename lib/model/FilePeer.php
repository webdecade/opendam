 ...
 # tag/fileSuccess
  ...
  public static function isUnderFolder($file_id, $folder_id)
  {
		...
		$uploadedDates = Array())
	{
		...
		function strictProcessFileIndex($operator, $array)
		{
			$return = Array();
			switch($operator)
			{
				...
				case "AND,":
				{
					...
					foreach($array as $values)
					{
						...
						*/
						if(!empty($oldValue)){
							$oldValue = array_intersect($oldValue, $values);
						}else{
							$oldValue = $values;
						}
					}
			...
			return array_unique($oldValue);
		}
		
		function masterFiles($array, $old, $keywords)
		{
			//$return = Array();
			
			foreach($array as $keyworder => $values)
			{
				if($keywords[0] == $keyworder){
					$plus = 2;	
				}else{
					$plus = 1;	
				}
				...
				{
					if(isset($old[$value]) && $old[$value] != 0){
						$old[$value] = $old[$value] + $plus;
					}else{
						$old[$value] = $plus;
					}
				}
		...
		function processFileIndexTags($operator, $array)
		{
			$return = Array();
			...
			switch($operator)
			{
				...
				case "AND":
				{
					...
					foreach($array as $values)
					{
						...
						*/
						if(!empty($oldValue))
							$return = array_intersect($return, array_intersect($oldValue, $values));
		...
		else
		{
			...
			if(!empty($keyword) && $keyword != __("search") && $keyword != __("Search"))
			{
				...
				**/
				...
				$old = masterFiles($temp, array(), $searchExpression);
				$idsTags = processFileIndex($searchOperator, $temp);
				if (strpos($keyword,',') !== false) {
    				$idsTags = strictProcessFileIndex($searchOperator, $temp);
    				//print_r($temp);print_r($idsTags);exit;
				}
				...
				**/
				...
				$old = masterFiles($temp, $old, $searchExpression);
			...
			$query .= " ORDER BY field(file.id,".implode($ids, ",").") ";
